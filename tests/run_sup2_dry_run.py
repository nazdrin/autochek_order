...
def latest_sup2_order(status: int | None = None) -> dict:
    ...
    return next(order for order in response.json().get("data", []) if str(order.get("supplierlist")) == "41")
...
def main() -> int:
    ...
    source = latest_sup2_order(args.status)
    ...
    run = subprocess.run(
        ...
        text=True,
        capture_output=True,
        timeout=max(180, args.pause_seconds + 180),
    )
